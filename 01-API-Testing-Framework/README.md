...
![Coverage](https://img.shields.io/badge/coverage-100%25-brightgreen)這是一個生產級 API 自動化測試框架，適合新手到進階自動化測試工程師學習與展示。你將循序漸進學會：環境安裝、API 測試設計、測試執行、報告產出與覆蓋率分析。

![Python](https://img.shields.io/badge/Python-3.12-blue)

# API Testing Framework 測試計畫

---

## 1. 專案目標與背景

本計畫旨在以 Python + pytest + requests 建立一套專業 API 自動化測試框架，涵蓋正向、負向、邊界與效能測試，並支援 HTML 報告、資料驅動、CI/CD、Docker 等現代化測試需求。適用於面試、作品集展示與團隊導入。

---

## 2. 測試範圍

- 目標 API：JSONPlaceholder Posts 端點
- 覆蓋功能：GET/POST/PUT/DELETE
- 測試類型：正向、負向、邊界、效能
- 不含認證、複雜業務邏輯
...
- Python 3.12+
- pytest 7.4, requests 2.31, pytest-html
- Postman / Newman
- Docker (可選)
...
| 編號 | 測試說明                 | 預期結果         |
|------|--------------------------|------------------|
| TC01 | 查詢所有 posts           | 200, 100 筆資料  |
| TC02 | 查詢單一 post (存在)     | 200, 正確內容    |
| TC03 | 查詢單一 post (不存在)   | 404              |
| TC04 | 新增 post (正確資料)     | 201, 回傳新資料  |
| TC05 | 新增 post (缺欄位)       | 400/201, 錯誤訊息|
| TC06 | 更新 post (存在)         | 200, 資料更新    |
| TC07 | 刪除 post (存在)         | 200, 資料刪除    |
| TC08 | API 回應時間             | < 1 秒           |
...
- **pytest fixture**：集中管理 API URL、headers、session，確保測試獨立與可重複執行
- **資料驅動/參數化**：減少重複、提升覆蓋率
- **自訂 marker**：支援冒煙、迴歸、效能等分組
- **AAA/FIRST 原則**：測試結構清晰、可維護
- **HTML 報告**：一鍵產生、易於展示
- **Docker/CI/CD**：確保環境一致、可自動化
...
- **命名規範**：test_功能_預期結果
- **自訂斷言訊息**：提升除錯效率
- **效能/Schema 驗證/Mock**：pytest-benchmark、jsonschema、responses
...
### Q4: 測試執行很慢
可用 pytest-xdist 並行、只執行失敗/修改過的測試。
...
### 10.1 展示流程建議
1. 展示專案結構與測試案例
2. 現場執行 pytest 並展示 HTML 報告
3. 強調設計亮點、覆蓋率、執行效率

### 10.2 技術問答重點
- 為何選 pytest？（語法簡潔、fixture、參數化、插件）
- 如何處理認證/測試資料？（fixture/參數化/清理）
...
**🎯 API Testing Framework - 展現專業測試工程能力**
...
| 測試案例數 | 8 | ✅ |    4. 每個項目包含必要欄位

| 測試通過率 | 100% (8/8) | ✅ |    """

| 測試覆蓋率 | 100% (CRUD + 異常) | ✅ |    # Act - 發送請求

| 執行時間 | < 3 秒 | ✅ |    response = api_client.get("/posts")

| 平均回應時間 | ~250ms | ✅ |    

| 報告完整性 | HTML + 詳細 Traceback | ✅ |    # Assert - 驗證回應
...
### 專案架構    assert isinstance(posts, list), "回應應為陣列"

    assert len(posts) == 100, f"應有 100 個項目，實際: {len(posts)}"

```    

01-API-Testing-Framework/    # 驗證資料結構

├── python-api-tests/    first_post = posts[0]

│   ├── tests/    required_fields = ["userId", "id", "title", "body"]

│   │   └── test_posts_api.py      # 8 個測試案例    for field in required_fields:

│   ├── test-reports/        assert field in first_post, f"缺少必要欄位: {field}"

│   │   └── api-report.html        # HTML 測試報告```

│   ├── conftest.py                # pytest 配置

│   ├── pytest.ini                 # pytest 設定#### 驗證重點

│   └── requirements.txt           # Python 依賴- ✅ **HTTP 狀態碼**: 確認 API 正確回應

├── postman/- ✅ **資料型別**: 驗證回傳 JSON 陣列

│   └── API-Test-Collection.json   # Postman 測試集合- ✅ **資料數量**: 確認完整性

└── README.md                       # 本文件- ✅ **資料結構**: 檢查必要欄位存在
...
### 面試展示重點Duration: 0.28s

Response Time: 245ms

當面試官問「你如何進行 API 測試」時：Status Code: 200 OK

Items Count: 100

1. **展示完整框架**（不只是 Postman 點擊）Data Validation: All fields present

2. **說明測試策略**（正向 60% + 負向 30% + 效能 10%）```

3. **展示專業報告**（HTML 格式，包含詳細資訊）

4. **強調測試覆蓋率**（100% CRUD + 異常處理）#### 面試展示要點

5. **突出進階技巧**（參數化、Fixture、自訂斷言）```
...
### 下一步4. 可擴展性高，未來可加入 JSON Schema 驗證」
...
1. ✅ 查看 [Web Automation](../02-Web-Automation-Framework/README.md) - 學習 UI 自動化測試

2. ✅ 學習 [Performance Testing](../03-Performance-Testing/README.md) - 深入效能測試<details>

3. ✅ 整合 [CI/CD](../04-CI-CD-Integration/README.md) - 自動化測試流程<summary><b>🔍 TC-API-003: 錯誤處理測試 - 展現負向測試思維</b></summary>

4. ✅ 使用 [Docker](../05-Docker-Test-Environment/README.md) - 容器化測試環境
...
<div align="center">#### 測試程式碼

```python

**🎯 API Testing Framework - 展現專業的測試工程能力**def test_get_non_existent_post(self, api_client):

    """

Made with ❤️ by Howie    負向測試：取得不存在的文章

    

[🏠 回到主頁](../README.md) | [📖 完整文檔](../06-Documentation/README.md) | [🔄 下一章](../02-Web-Automation-Framework/README.md)    測試場景: 請求不存在的 post ID
